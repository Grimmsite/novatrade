...
const REDIRECT_URI = process.env.REDIRECT_URI || 'https://novatrade-api.onrender.com/callback';
...
app.get('/callback', async (req, res) => {
  ...
  var cvMatch = (rawState.startsWith('CV.') && firstDot !== lastDot)
    ? [null, rawState.slice(3, lastDot), rawState.slice(lastDot + 1)]
    : null;
  var codeVerifier = cvMatch ? cvMatch[1] : null;
  var realState = cvMatch ? cvMatch[2] : rawState;

  if (error) return res.redirect(FRONTEND_URL + '/?auth_error=' + encodeURIComponent(error));
  if (!code || !codeVerifier) return res.redirect(FRONTEND_URL + '/?auth_error=missing_cv');
  ...
  try {
    ...
    if (!tokenRes.ok || !tokenData.access_token) {
      return res.redirect(FRONTEND_URL + '/?auth_error=' + encodeURIComponent(JSON.stringify(tokenData)));
    }
