...
function renderAccountSwitcher() {
  ...
  var html = '<div style="font-size:12px;color:#aaa;margin-bottom:6px">Switch Account</div>';
  accounts.forEach(function(a) {
    var active = a.account_id === state.accountId;
    var label = (a.account_type === 'real' ? 'Real' : 'Demo') + ' — ' + a.account_id + ' (' + a.currency + ')';
    html += '<button onclick="switchAccount('' + a.account_id + '')" style="display:block;width:100%;text-align:left;padding:8px 12px;margin-bottom:4px;border-radius:6px;border:1px solid ' + (active ? '#f0c040' : '#444') + ';background:' + (active ? '#2a2200' : '#1a1a1a') + ';color:#fff;cursor:pointer">' + label + (active ? ' ✓' : '') + '</button>';
  });
  el.innerHTML = html;
}
...
async function switchAccount(accountId) {
  ...
  state.accountId = accountId;
  localStorage.setItem('nt_account_id', accountId);
  var acct = (state.allAccounts || []).find(function(a) { return a.account_id === accountId; }) || {};
  state.currency = acct.currency || state.currency;
  renderAccountSwitcher();
  await fetchBalance();
  showToast('Switched to ' + (acct.account_type === 'real' ? 'Real' : 'Demo') + ' account');
}
