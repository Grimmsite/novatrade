...
function updateAtoolDisplay() {
  ...
  try {
    ...
    accounts.forEach(function(a) {
      ...
      var check = active ? " ✓" : "";
      var btn = "<button onclick='switchAccount(\"" + a.account_id + "\")' style='display:block;width:100%;text-align:left;padding:8px 12px;margin-bottom:4px;border-radius:6px;border:1px solid " + border + ";background:" + bg + ";color:#fff;cursor:pointer'>" + label + check + "</button>";
      html += btn;
    });
    el.innerHTML = html;
  } catch(e) { console.warn("switcher error:", e); }
};
