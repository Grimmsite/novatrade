...
var BOT_LIBRARY = [
  { id: 'rise_fall_pro', name: 'Rise Fall Pro', desc: 'Martingale CALL — R_50, 3 ticks', risk: 'MEDIUM', icon: '📈',
    config: { symbol: 'R_50', duration: 3, duration_unit: 't', contract_type: 'CALL', barrier: null, initial_stake: 1, multiplier: 2.1 } },
  { id: 'even_odd_master', name: 'Even Odd Master', desc: 'Martingale DIGITEVEN — R_100, 1 tick', risk: 'MEDIUM', icon: '🎯',
    config: { symbol: 'R_100', duration: 1, duration_unit: 't', contract_type: 'DIGITEVEN', barrier: null, initial_stake: 1, multiplier: 2.15 } },
  { id: 'over_under_elite', name: 'Over Under Elite', desc: 'Martingale DIGITOVER barrier 4 — R_100, 1 tick', risk: 'HIGH', icon: '⚡',
    config: { symbol: 'R_100', duration: 1, duration_unit: 't', contract_type: 'DIGITOVER', barrier: 4, initial_stake: 1, multiplier: 2.3 } },
  { id: 'sniper', name: 'Sniper', desc: 'Martingale DIGITOVER barrier 4 x2.2 — R_100, 1 tick', risk: 'HIGH', icon: '🎯',
    config: { symbol: 'R_100', duration: 1, duration_unit: 't', contract_type: 'DIGITOVER', barrier: 4, initial_stake: 1, multiplier: 2.2 } },
  { id: 'volatility_hunter', name: 'Volatility Hunter', desc: 'CALL on R_75, 5 ticks, base stake 2', risk: 'LOW', icon: '🔥',
    config: { symbol: 'R_75', duration: 5, duration_unit: 't', contract_type: 'CALL', barrier: null, initial_stake: 2, multiplier: 2.1 } }
];

var botState = {
  running: false, selectedBot: null, currentStake: 1, initialStake: 1,
  sessionPnl: 0, trades: 0, wins: 0, losses: 0,
  ws: null, pendingContractId: null, stopped: false,
  takeProfit: 10, stopLoss: 10, maxTrades: 100
};

function renderBotLibrary() {
  var el = document.getElementById('botLibraryList');
  if (!el) return;
  el.innerHTML = BOT_LIBRARY.map(function(bot) {
    var riskColor = bot.risk === 'LOW' ? '#4caf50' : bot.risk === 'MEDIUM' ? '#f0c040' : '#f44336';
    var active = botState.selectedBot && botState.selectedBot.id === bot.id;
    return '<div class="bot-card' + (active ? ' active' : '') + '" onclick="selectBot('' + bot.id + '')">'
      + '<div class="bot-card-icon">' + bot.icon + '</div>'
      + '<div class="bot-card-info"><div class="bot-card-name">' + bot.name + '</div>'
      + '<div class="bot-card-desc">' + bot.desc + '</div></div>'
      + '<span class="bot-risk-badge" style="color:' + riskColor + ';border-color:' + riskColor + '">' + bot.risk + '</span>'
      + '</div>';
  }).join('');
}
...
function selectBot(id) {
  ...
  botState.selectedBot = BOT_LIBRARY.find(function(b){ return b.id === id; }) || null;
  renderBotLibrary();
  var nameEl = document.getElementById('activeBotName');
  var runBtn = document.getElementById('botRunBtn');
  if (nameEl) nameEl.textContent = botState.selectedBot ? botState.selectedBot.name : 'No bot selected';
  if (runBtn) runBtn.disabled = !botState.selectedBot || !state.bearerToken;
  setBotStatus('idle', botState.selectedBot ? 'Ready — press Run to start' : 'Select a bot and press Run');
}

function loadCustomXML(event) {
  var file = event.target.files[0];
  if (!file) return;
  var reader = new FileReader();
  reader.onload = function(e) {
    var xml = e.target.result;
    var config = parseXMLConfig(xml);
    if (!config) { showToast('Could not parse XML bot'); return; }
    var customBot = { id: 'custom_' + Date.now(), name: file.name.replace('.xml',''),
      desc: 'Custom uploaded bot', risk: 'MEDIUM', icon: '📂', config: config };
    BOT_LIBRARY.push(customBot);
    renderBotLibrary();
    selectBot(customBot.id);
    showToast('Bot loaded: ' + customBot.name);
  };
  reader.readAsText(file);
}

function parseXMLConfig(xmlStr) {
  try {
    var parser = new DOMParser();
    var doc = parser.parseFromString(xmlStr, 'text/xml');
    var marketBlock = doc.querySelector('block[type="trade_definition_market"]');
    var symbol = marketBlock ? ((marketBlock.querySelector('field[name="SYMBOL"]') || {}).textContent || 'R_100') : 'R_100';
    var tradeOpts = doc.querySelector('block[type="trade_definition_tradeoptions"]');
    var duration = tradeOpts ? parseInt((tradeOpts.querySelector('field[name="DURATION_VALUE"]') || {}).textContent || '1') : 1;
    var duration_unit = tradeOpts ? ((tradeOpts.querySelector('field[name="DURATION_UNIT"]') || {}).textContent || 't') : 't';
    var purchaseBlock = doc.querySelector('block[type="purchase"]');
    var contract_type = purchaseBlock ? ((purchaseBlock.querySelector('field[name="PURCHASE_LIST"]') || {}).textContent || 'CALL') : 'CALL';
    var barrier = null;
    if (purchaseBlock) {
      var bv = purchaseBlock.querySelector('value[name="BARRIER"] block[type="math_number"] field[name="NUM"]');
      if (bv) barrier = parseFloat(bv.textContent);
    }
    var initial_stake = 1;
    var bp = doc.querySelector('statement[name="BEFORE_PURCHASE_STATEMENT"]');
    if (bp) { var nums = bp.querySelectorAll('field[name="NUM"]'); if (nums.length > 0) initial_stake = parseFloat(nums[0].textContent) || 1; }
    var multiplier = 2.0;
    var ap = doc.querySelector('statement[name="AFTER_PURCHASE_STATEMENT"]');
    if (ap) {
      var mb = ap.querySelector('block[type="math_arithmetic"] field[name="OP"]');
      if (mb && mb.textContent === 'MULTIPLY') {
        var mn = ap.querySelectorAll('block[type="math_arithmetic"] field[name="NUM"]');
        if (mn.length > 0) multiplier = parseFloat(mn[0].textContent) || 2.0;
      }
    }
    return { symbol: symbol, duration: duration, duration_unit: duration_unit,
             contract_type: contract_type, barrier: barrier, initial_stake: initial_stake, multiplier: multiplier };
  } catch(e) { console.error('XML parse error:', e); return null; }
}

function setBotStatus(type, text) {
  var dot = document.getElementById('botStatusDot');
  var txt = document.getElementById('botStatusText');
  if (dot) dot.className = 'status-dot ' + type;
  if (txt) txt.textContent = text;
}

function addTradeLog(entry) {
  var log = document.getElementById('botTradeLog');
  if (!log) return;
  var placeholder = log.querySelector('.log-placeholder');
  if (placeholder) placeholder.remove();
  var color = entry.result === 'win' ? '#4caf50' : '#f44336';
  var row = document.createElement('div');
  row.className = 'log-row';
  row.innerHTML = '<span class="log-time">' + new Date().toLocaleTimeString() + '</span>'
    + '<span class="log-contract">' + entry.contract_type + '</span>'
    + '<span class="log-stake">$' + entry.stake.toFixed(2) + '</span>'
    + '<span class="log-result" style="color:' + color + '">' + entry.result.toUpperCase() + '</span>'
    + '<span class="log-pnl" style="color:' + color + '">' + (entry.profit >= 0 ? '+' : '') + '$' + entry.profit.toFixed(2) + '</span>';
  log.insertBefore(row, log.firstChild);
  while (log.children.length > 50) log.removeChild(log.lastChild);
}

function updateBotStats() {
  var pnlEl = document.getElementById('stat_pnl');
  if (document.getElementById('stat_trades')) document.getElementById('stat_trades').textContent = botState.trades;
  if (document.getElementById('stat_wins')) document.getElementById('stat_wins').textContent = botState.wins;
  if (document.getElementById('stat_losses')) document.getElementById('stat_losses').textContent = botState.losses;
  if (document.getElementById('stat_stake')) document.getElementById('stat_stake').textContent = '$' + botState.currentStake.toFixed(2);
  if (pnlEl) {
    pnlEl.textContent = (botState.sessionPnl >= 0 ? '+' : '') + '$' + botState.sessionPnl.toFixed(2);
    pnlEl.style.color = botState.sessionPnl >= 0 ? '#4caf50' : '#f44336';
  }
}

async function startBot() {
  if (!state.bearerToken || !state.accountId) { showToast('Please log in first'); return; }
  if (!botState.selectedBot) { showToast('Select a bot first'); return; }
  if (botState.running) return;
  botState.initialStake = parseFloat(document.getElementById('bs_stake').value) || 1;
  botState.currentStake = botState.initialStake;
  botState.takeProfit = parseFloat(document.getElementById('bs_tp').value) || 10;
  botState.stopLoss = parseFloat(document.getElementById('bs_sl').value) || 10;
  botState.maxTrades = parseInt(document.getElementById('bs_max').value) || 100;
  botState.sessionPnl = 0; botState.trades = 0; botState.wins = 0; botState.losses = 0;
  botState.stopped = false; botState.running = true;
  document.getElementById('botRunBtn').classList.add('hidden');
  document.getElementById('botStopBtn').classList.remove('hidden');
  document.getElementById('botTradeLog').innerHTML = '';
  updateBotStats();
  setBotStatus('running', 'Connecting...');
  try {
    var wsUrl = await getAuthWsUrl(state.bearerToken, state.accountId);
    botState.ws = new WebSocket(wsUrl);
    botState.ws.onopen = function() { setBotStatus('running', 'Running — ' + botState.selectedBot.name); runNextTrade(); };
    botState.ws.onmessage = function(e) { handleBotMessage(JSON.parse(e.data)); };
    botState.ws.onerror = function() { stopBot(); showToast('WebSocket error — bot stopped'); };
    botState.ws.onclose = function() { if (botState.running) { setBotStatus('idle', 'Connection closed'); botState.running = false; } };
  } catch(err) { stopBot(); showToast('Failed to connect: ' + err.message); }
}

function runNextTrade() {
  if (botState.stopped || !botState.running) return;
  if (botState.trades >= botState.maxTrades) { stopBot(); showToast('Max trades reached'); return; }
  if (botState.sessionPnl <= -Math.abs(botState.stopLoss)) { stopBot(); showToast('Stop loss hit'); return; }
  if (botState.sessionPnl >= Math.abs(botState.takeProfit)) { stopBot(); showToast('Take profit hit'); return; }
  var cfg = botState.selectedBot.config;
  var proposal = { proposal: 1, amount: parseFloat(botState.currentStake.toFixed(2)), basis: 'stake',
    contract_type: cfg.contract_type, currency: 'USD', duration: cfg.duration,
    duration_unit: cfg.duration_unit, symbol: cfg.symbol };
  if (cfg.barrier !== null && cfg.barrier !== undefined) proposal.barrier = cfg.barrier;
  setBotStatus('running', 'Trade #' + (botState.trades + 1) + ' — stake $' + botState.currentStake.toFixed(2));
  botState.ws.send(JSON.stringify(proposal));
}

function handleBotMessage(msg) {
  if (msg.error) {
    console.error('Bot error:', msg.error.message);
    setBotStatus('error', 'Error: ' + msg.error.message);
    setTimeout(function(){ if (botState.running) runNextTrade(); }, 2000);
    return;
  }
  if (msg.msg_type === 'proposal' && msg.proposal) {
    botState.ws.send(JSON.stringify({ buy: msg.proposal.id, price: msg.proposal.ask_price }));
    return;
  }
  if (msg.msg_type === 'buy' && msg.buy) {
    botState.pendingContractId = msg.buy.contract_id;
    setBotStatus('running', 'Contract open — waiting for result...');
    botState.ws.send(JSON.stringify({ proposal_open_contract: 1, contract_id: botState.pendingContractId, subscribe: 1 }));
    return;
  }
  if (msg.msg_type === 'proposal_open_contract' && msg.proposal_open_contract) {
    ...
    var stake = parseFloat(poc.buy_price) || botState.currentStake;
    botState.trades++; botState.sessionPnl += profit;
    if (won) { botState.wins++; botState.currentStake = botState.initialStake; }
    else { botState.losses++; botState.currentStake = parseFloat((botState.currentStake * botState.selectedBot.config.multiplier).toFixed(2)); }
    addTradeLog({ contract_type: botState.selectedBot.config.contract_type, stake: stake, result: won ? 'win' : 'loss', profit: profit });
    updateBotStats();
    setTimeout(function(){ if (botState.running && !botState.stopped) runNextTrade(); }, 500);
  }
}

function stopBot() {
  botState.running = false; botState.stopped = true;
  if (botState.ws) { try { botState.ws.close(); } catch(e){} botState.ws = null; }
  var runBtn = document.getElementById('botRunBtn');
  var stopBtn = document.getElementById('botStopBtn');
  if (runBtn) runBtn.classList.remove('hidden');
  if (stopBtn) stopBtn.classList.add('hidden');
  setBotStatus('idle', 'Stopped — ' + botState.trades + ' trades, P&L: ' + (botState.sessionPnl >= 0 ? '+' : '') + '$' + botState.sessionPnl.toFixed(2));
}

document.addEventListener('DOMContentLoaded', function(){ renderBotLibrary(); });
